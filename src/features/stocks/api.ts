...
export interface CreerStockPayload {
...
export async function creerStock(payload: CreerStockPayload) {
  const reponse = await api.post("/stock", payload);
  return reponse.data?.data ?? reponse.data;
}

export async function ajouterEntree(
  stockId: number,
  payload: { quantite: number; motif?: string }
) {
  const reponse = await api.patch(`/stock/${stockId}/entree`, payload);
  return reponse.data?.data ?? reponse.data;
}
...
export async function ajusterStock(
  ...
  const reponse = await api.patch(`/stock/${stockId}/ajustement`, payload);
  return reponse.data?.data ?? reponse.data;
}

export async function modifierSeuil(
  stockId: number,
  seuil_alerte: number
) {
  const reponse = await api.patch(`/stock/${stockId}/seuil`, { seuil_alerte });
  return reponse.data?.data ?? reponse.data;
}
